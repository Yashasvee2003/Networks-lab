...
int main(int argc, char *argv[]) {
  ...
  int n = recvfrom(server_socket, domain, sizeof(domain), 0, (struct sockaddr*) &client_addr, &client_len);

  printf("%d", n);
  // domain[n] = '\0';
  printf("domain name got from client : %s", domain);

  strcpy(new_ip, "1.1.1.1");


   sendto(server_socket, new_ip, strlen(new_ip), 0, (struct sockaddr*) &client_addr, client_len);
